...
// Close all S1 orders across ALL 7 TF when M1 signal changes
// TRIGGER: M1 signal change + S1_CloseByM1 == true
// ACTION: Close magic[tf][0] for ALL 7 TF
void CloseS1OrdersByM1() {
    for(int tf = 0; tf < 7; tf++) {
        if(!IsTFEnabled(tf)) continue;

        int target_magic = g_ea.magic_numbers[tf][0];  // S1 magic
        ...
        for(int i = PositionsTotal() - 1; i >= 0; i--) {
            ...
            if(PositionGetInteger(POSITION_MAGIC) == target_magic) {
                CloseOrderSafely((int)ticket, "S1_M1_CLOSE");
            }
        }

        g_ea.position_flags[tf][0] = 0;  // Reset S1 flag
    }
}

// Close all S2 orders across ALL 7 TF when M1 signal changes
// TRIGGER: M1 signal change + S2_CloseByM1 == true
// ACTION: Close magic[tf][1] for ALL 7 TF
void CloseS2OrdersByM1() {
    for(int tf = 0; tf < 7; tf++) {
        if(!IsTFEnabled(tf)) continue;

        int target_magic = g_ea.magic_numbers[tf][1];  // S2 magic
        ...
        for(int i = PositionsTotal() - 1; i >= 0; i--) {
            ...
            if(PositionGetInteger(POSITION_MAGIC) == target_magic) {
                CloseOrderSafely((int)ticket, "S2_M1_CLOSE");
            }
        }

        g_ea.position_flags[tf][1] = 0;  // Reset S2 flag
    }
}

// Close only S3 orders for specific TF (when S1/S2 already closed by M1)
// TRIGGER: TF signal change, but S1/S2 already closed by M1
// ACTION: Close only magic[tf][2] for this specific TF
void CloseS3OrdersForTF(int tf) {
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        if(PositionGetInteger(POSITION_MAGIC) == target_magic) {
            CloseOrderSafely((int)ticket, "S3_SIGNAL_CHG");
        }
    }

    g_ea.position_flags[tf][2] = 0;  // Reset S3 flag
}
    ...
    for(int i = 0; i < PositionsTotal(); i++) {
        ...
        for(int tf = 0; tf < 7; tf++) {
            ...
            if(magic == bonus_magic) {
                ...
                bonus_lots_per_tf[tf] += PositionGetDouble(POSITION_VOLUME);
                found = true;
                break;
            }
        }
    ...
    if(!UseEvenOddMode || (current_second % 2 == 0)) {
        ...
        for(int tf = 0; tf < 7; tf++) {
            if(HasValidS2BaseCondition(tf)) {
                ...
                // STEP 3.1: FAST CLOSE by M1 (when M1 signal changes)
                // LOGIC: When M1 (tf==0) signal changes, close S1/S2/BONUS across ALL 7 TF immediately
                // LOGIC: Khi tin hieu M1 thay doi, dong S1/S2/BONUS tren TAT CA 7 khung ngay lap tuc
                if(tf == 0 && HasValidS2BaseCondition(0)) {
                    if(S1_CloseByM1) CloseS1OrdersByM1();       // Close S1 for ALL 7 TF
                    if(S2_CloseByM1) CloseS2OrdersByM1();       // Close S2 for ALL 7 TF
                    if(EnableBonusNews) CloseAllBonusOrders();  // Close BONUS for ALL 7 TF
                }

                // STEP 3.2: NORMAL CLOSE by TF (4 modes based on S1/S2 settings)
                // MODE 1: Both S1+S2 close by M1 ? Only close S3 for this TF
                // MODE 2: Only S1 closes by M1 ? Close S2+S3 for this TF
                // MODE 3: Only S2 closes by M1 ? Close S1+S3 for this TF
                // MODE 4: Neither closes by M1 ? Close ALL 3 strategies for this TF (original)
                if(S1_CloseByM1 && S2_CloseByM1) {
                    // MODE 1: S1 & S2 already closed by M1 ? Only close S3
                    CloseS3OrdersForTF(tf);
                } else if(S1_CloseByM1) {
                    // MODE 2: S1 already closed by M1 ? Close S2 + S3
                    for(int i = PositionsTotal() - 1; i >= 0; i--) {
                        ulong ticket = PositionGetTicket(i);
                        if(ticket == 0) continue;
                        if(PositionGetString(POSITION_SYMBOL) != Symbol()) continue;
                    ...
                    // MODE 3: S2 already closed by M1 ? Close S1 + S3
                    ...
                    // MODE 4: Original behavior - close ALL 3 strategies
